...
def crop_face(img, gpu_memory_fraction=0.7, random_order=False, detect_multiple_faces=True, margin=32):
    ...
    if nrof_faces>0:
        ...
        for i, det in enumerate(det_arr):
            ...
            imageio.imsave("image_112.png", scaled_112)
            imageio.imsave("image_160.png", scaled_160)
    return bounding_boxes_list

def vectorizer(image_112_path, image_160_path, model_mxnet_path, model_facenet_path):
    model_mxnet = get_model_mxnet(model_mxnet_path)
    model_facenet = get_model_facenet(model_facenet_path)
    img_test_path_mxnet = imageio.imread(image_112_path)
    img_test_path_facenet = imageio.imread(image_160_path)
    db_emb = get_feature_mxnet(model_mxnet, img_test_path_mxnet)
    db_emb = db_emb.reshape((1,512))

    db_emb_2 = get_feature_facenet(model_facenet, img_test_path_facenet)
    db_emb_2 = db_emb_2.reshape((1,128))
    db_emb = np.concatenate((db_emb, db_emb_2), axis=1)
    return db_emb

def get_same_person(find_emb, emb_vectors_list_path, threshold):
    list_vectors = pickle.load(open(emb_vectors_list_path, 'rb'))
    list_result = []
    dict_check = []
    for i in range(0, len(list_vectors)):
        ...
        if (dist < threshold):
            if name not in dict_check:
                list_result.append([name, list_vectors[i][1]])
